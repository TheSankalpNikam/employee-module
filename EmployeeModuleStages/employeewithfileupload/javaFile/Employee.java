...
public class Employee extends Base 
{
    ...
    public ArrayList getEmployeeByName(String search, int statusIndex, int next, int count)
    {
        ...
        try
        {
            ...
            String name, lastname, emailid, number, countryName, cityName, description, empfilename;
    ...
    public EmployeeInfo getEmployeeDetailById(int employeeId)
    {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT t_employee.s_name, t_employee.s_lastname, t_employee.s_emailid, t_employee.s_number, t_employee.i_countryid, t_city.s_name , t_employee.s_description, t_employee.i_status, t_employee.s_empfilename FROM t_employee  ");
        sb.append("left join t_city on (t_employee.i_cityid = t_city.i_cityid) ");
        ...
        try
        {
            ...
            rs = pstmt.executeQuery();
            String name, lastname, emailid, number, cityName, description , empfilename;
            ...
            while (rs.next())
            {
                ...
                info = new EmployeeInfo(employeeId, name, lastname, emailid, number, countryId, cityName, description ,status, empfilename,0); 
            }
        }
    ...
    public EmployeeInfo getEmployeeDetailByIdforDetail(int employeeId)
    {
        EmployeeInfo info = null;
        StringBuilder sb = new StringBuilder();
        sb.append("select t_employee.s_name, t_employee.s_lastname, t_employee.s_emailid, t_employee.s_number, t_country.s_name, t_city.s_name, t_employee.s_description, t_employee.i_status, t_employee.s_empfilename from t_employee ");
        sb.append("left join t_country on (t_employee.i_countryid = t_country.i_countryid) ");
        sb.append("left join t_city on (t_employee.i_cityid = t_city.i_cityid) ");
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                info = new EmployeeInfo(employeeId, name, lastname, emailid, number, countryName, cityName, description, status, empfilename,0); 
            }
            rs.close();            
        }
        catch (Exception exception)
    ...
    public int createEmployee(EmployeeInfo info)
    {
        ...
        try
        {
            ...
            sb.append("(s_name, s_lastname, s_emailid , s_number, i_countryid, i_cityid, s_description, i_status, s_empfilename, i_userid, ts_regdate, ts_moddate) ");
            sb.append("values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            String query = (sb.toString()).intern();
            sb.setLength(0);

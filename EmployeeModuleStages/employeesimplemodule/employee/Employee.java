...
public class Employee extends Base 
{
    ...
    public ArrayList getEmployeeByName(String search, int statusIndex, int next, int count)
    {
        ...
        sb.append("select i_employeeid, s_name, s_fathername,s_emailid, s_number, i_status ");
        sb.append("FROM t_employee where 0 = 0 ");
        if(search != null && !search.equals(""))
            sb.append("and (s_name like ? OR s_s_fathername like ? or s_number = ?) ");
        if(statusIndex > 0)
        {
        ...
        sb.append("select count(1) FROM t_employee where 0 = 0 ");
        if(search != null && !search.equals(""))
            sb.append("and (s_name like ? OR s_fathername like ? or s_number = ?) ");
        if(statusIndex > 0)
        {
        ...
        try
        {
            ...
            String name, fathername, emailid, number;
            ...
            while (rs.next())
            {
                ...
                status = rs.getInt(6);
                employees.add(new EmployeeInfo(employeeId, name, fathername, emailid, number, status, 0));
            }
            rs.close();
    ...
    public EmployeeInfo getEmployeeDetailById(int employeeId)
    {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT s_name, s_fathername,s_emailid, s_number, i_status FROM t_employee where i_employeeid = ? ");
        String query = sb.toString().intern();
        sb.setLength(0);
        try
        {
            ...
            while (rs.next())
            {
                ...
                status = rs.getInt(5);
                info = new EmployeeInfo(employeeId, name, fathername, emailid, number, status, 0); 
            }
        }
    ...
    public int createEmployee(EmployeeInfo info)
    {
        ...
        try
        {
            ...
            sb.append("(s_name, s_fathername, s_emailid , s_number, i_status, i_userid, ts_regdate, ts_moddate) ");
            sb.append("values (?, ?, ?, ?, ?, ?, ?, ?)");
            String query = (sb.toString()).intern();
            sb.setLength(0);
            ...
            pstmt.setString(++scc, (info.getFathername()));
    ...
    public int checkDuplicacy(int employeeId, String name, String fathername)
    {
        int ck = 0;
        StringBuilder sb = new StringBuilder();
        sb.append("select i_employeeid FROM t_employee where (s_name = ? OR s_fathername = ?) and i_status in (1, 2)");
        if(employeeId > 0)
        ...
        try
        {
            ...
            pstmt.setString(++scc, fathername);
            if(employeeId > 0)
    ...
    public ArrayList getListForExcel(String search, int statusIndex) 
    {
        ...
        StringBuilder sb = new StringBuilder();
         sb.append("select s_name, s_fathername, s_emailid, s_number, i_status ");
        sb.append("FROM t_country where 0 = 0 ");
        if(search != null && !search.equals(""))
            sb.append("and (s_name like ? OR s_fathername like ? or s_emailid = ?) ");
        if(statusIndex > 0)
        {
            sb.append("and i_status = ? ");
        }
        ...
        try {
           ...
           if(search != null && !search.equals(""))
            ...
            while (rs.next())
            {
                ...
                number = rs.getString(4);
                status = rs.getInt(5);
                list.add(new EmployeeInfo(0, name, fathername, emailid, number, status, 0));
            }
            rs.close();
            pstmt.close();            
        } catch (Exception e) {
    ...
    {
       String query = ("Select i_employeeid, s_name, s_fathername, s_emailid, s_number FROM t_employee where i_status = 1 order by  s_name ");
        try
        {
            pstmt = conn.prepareStatement(query);
            rs = pstmt.executeQuery();
            ...
            while (rs.next())
            {
                ...
                jobj.put("id", id);
                jobj.put("name", name);
                jobj.put("fathername", fathername);

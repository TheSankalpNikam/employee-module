...
public class Employee extends Base 
{
    ...
    public ArrayList getEmployeeByName(String search, int statusIndex, int next, int count)
    {
        ...
        sb.append("select t_employee.i_employeeid, t_employee.s_name, t_employee.s_lastname, t_employee.s_emailid, t_employee.s_number, t_country.s_name, t_city.s_name, t_employee.i_status from t_employee ");
        ...
        if(search != null && !search.equals(""))
            sb.append("and (t_employee.s_name like ? OR t_employee.s_lastname like ? or t_employee.s_number = ? OR t_country.s_name like ? OR t_city.s_name like ?) ");
        if(statusIndex > 0)
        {
        ...
        try
        {
            ...
            String name, lastname, emailid, number, countryName, cityName;
            ...
            while (rs.next())
            {
                ...
                status = rs.getInt(8);
                employees.add(new EmployeeInfo(employeeId, name, lastname, emailid, number, countryName, cityName ,status, 0));
            }
            rs.close();
    ...
    public EmployeeInfo getEmployeeDetailById(int employeeId)
    {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT s_name, s_lastname,s_emailid, s_number, i_countryid, i_cityid ,i_status FROM t_employee where i_employeeid = ? ");
        String query = sb.toString().intern();
        sb.setLength(0);
        try
        {
            ...
            while (rs.next())
            {
                ...
                cityId = rs.getInt(6);
                status = rs.getInt(7);
                info = new EmployeeInfo(employeeId, name, lastname, emailid, number, countryId, cityId, status, 0); 
            }
        }
    ...
    public EmployeeInfo getEmployeeDetailByIdforDetail(int employeeId)
    {
        EmployeeInfo info = null;
        StringBuilder sb = new StringBuilder();
        sb.append("select t_employee.s_name, t_employee.s_lastname, t_employee.s_emailid, t_employee.s_number, t_country.s_name, t_city.s_name, t_employee.i_status from t_employee ");
        sb.append("left join t_country on (t_employee.i_countryid = t_country.i_countryid) ");
        sb.append("left join t_city on (t_employee.i_cityid = t_city.i_cityid) ");
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                emailid = rs.getString(3);
                number = rs.getString(4);
                countryName = rs.getString(5);
                cityName = rs.getString(6);
                status = rs.getInt(7); 
                System.out.println("cityName :: " + cityName);
                info = new EmployeeInfo(employeeId, name, lastname, emailid, number, countryName, cityName, status, 0); 
            }
            rs.close();            
        }
        catch (Exception exception)
    ...
    public int createEmployee(EmployeeInfo info)
    {
        ...
        try
        {
            ...
            sb.append("(s_name, s_lastname, s_emailid , s_number, i_countryid, i_cityid, i_status, i_userid, ts_regdate, ts_moddate) ");
            sb.append("values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            String query = (sb.toString()).intern();
            sb.setLength(0);
    ...
    {
       String query = ("Select i_employeeid, s_name, s_lastname, s_emailid, s_number FROM t_employee where i_status = 1 order by  s_name ");
        try
        {
            pstmt = conn.prepareStatement(query);
            rs = pstmt.executeQuery();
